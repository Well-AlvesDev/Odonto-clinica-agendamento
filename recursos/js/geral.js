...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    try {
        ...
        // ===================================================
        // LIMPAR DATAS PASSADAS (MÁXIMO 3)
        // ===================================================
        await limparDatasPassadas();

        await carregarServicosClinica();
        await carregarDuracoesServicos(); // Carregar durações e preços dos serviços
...
async function carregarDadosDoServidor() {
    try {
        ...
        // Atualizar quantidade de datas no título
        const quantidadeElement = document.getElementById('quantidadeDatas');
        if (quantidadeElement) {
            const quantidade = horariosGlobais.length;
            quantidadeElement.textContent = `(${quantidade} ${quantidade === 1 ? 'data' : 'datas'})`;
        }
...
// ===================================================
// LIMPAR DATAS PASSADAS (MANTER MÁXIMO 3)
// ===================================================
async function limparDatasPassadas() {
    try {
        ...
        if (!usuarioLogado || !senhaUsuario) {
            console.warn('Credenciais não encontradas para limpeza de datas passadas');
            return;
        }

        // Chamar função SQL para limpar datas passadas
        const { data: resultadoLimpeza, error: erroLimpeza } = await _supabase.rpc(
            'limpar_datas_passadas_manter_3',
            {
        ...
        // Verificar erro
        if (erroLimpeza) {
            console.error('Erro ao limpar datas passadas:', erroLimpeza);
            return;
        }

        // Verificar resposta da função SQL
        if (resultadoLimpeza && resultadoLimpeza.sucesso) {
            if (resultadoLimpeza.datas_removidas > 0) {
                console.warn(
                    `⚠️ Limpeza de datas passadas: ${resultadoLimpeza.datas_removidas} datas removidas`,
                    resultadoLimpeza.datas_removidas_lista
                );

                // Recarregar dados após limpeza
                await carregarDadosDoServidor();
            } else {
                console.log('Nenhuma data passada para remover (dentro do limite de 3)');
            }
        } else if (resultadoLimpeza && !resultadoLimpeza.sucesso) {
            console.error('Erro na limpeza:', resultadoLimpeza.mensagem);
        }
    } catch (erro) {
        console.error('Erro inesperado ao limpar datas passadas:', erro);
    }
}
...
let servicoDetalhesAtual = null; // serviço atualmente visualizado no modal de detalhes
let campoEdicaoAtual = null; // 'duracao' ou 'preco'
...
async function abrirModalDetalhesServico(nomeServico) {
    ...
    servicoDetalhesAtual = nomeServico;

    try {
        const duracao = await obterDuracaoServico(nomeServico);
...
function iniciarEdicaoServico(campo) {
    if (!servicoDetalhesAtual) {
        mostrarErro('Serviço não definido. Reabra o modal e tente novamente.');
        return;
    }
    ...
    if (isDuracao) {
        label.textContent = 'Nova duração (minutos)';
        inputValor.type = 'number';
        inputValor.step = '1';
        inputValor.min = '1';
        inputValor.value = isFinite(duracaoServico) && duracaoServico > 0 ? duracaoServico : '';
    } else {
        label.textContent = 'Novo preço (R$)';
        inputValor.type = 'number';
        inputValor.step = '0.01';
        inputValor.min = '0';
        inputValor.value = isFinite(precoServico) && precoServico >= 0 ? precoServico : '';
    }

    document.getElementById('modalOverlayEdicaoServico').classList.add('ativo');
    btnSalvar.disabled = false;
...
async function salvarEdicaoServico() {
    if (!servicoDetalhesAtual || !campoEdicaoAtual) {
        mostrarErro('Nenhuma edição selecionada.');
        return;
    }

    const valorInput = document.getElementById('inputEditarServicoValor').value.trim();
    const usuarioAdmin = document.getElementById('inputAdminNome').value.trim();
    const senhaAdmin = document.getElementById('inputAdminSenha').value.trim();

    if (!valorInput) {
        mostrarErro('Informe o valor que deseja alterar.');
        return;
    }
    if (!usuarioAdmin || !senhaAdmin) {
        mostrarErro('Informe usuário e senha do admin.');
        return;
    }
    ...
    if (campoEdicaoAtual === 'duracao') {
        valorConversao = parseInt(valorInput, 10);
        if (!Number.isInteger(valorConversao) || valorConversao <= 0) {
            mostrarErro('Informe uma duração válida em minutos (> 0).');
            return;
        }
        campoDB = 'duracao_minuto';
    } else {
        valorConversao = parseFloat(valorInput.replace(',', '.'));
        if (isNaN(valorConversao) || valorConversao < 0) {
            mostrarErro('Informe um preço válido (>= 0).');
            return;
        }
        campoDB = 'price';
    }
    ...
    btnSalvar.disabled = true;
    btnSalvar.classList.add('loading');
    btnSalvar.innerHTML = '<i class="ri-loader-4-line"></i> Salvando...';

    try {
        const { data, error } = await _supabase.rpc('atualizar_tempo_servico_por_admin', {
            p_nome_usuario: usuarioAdmin,
            p_senha_usuario: senhaAdmin,
            p_servico: servicoDetalhesAtual,
            p_campo: campoDB,
            p_valor: valorConversao
        });
        ...
        mostrarErro('Erro ao atualizar: ' + (erro.message || 'verifique credenciais e valores.'));
    } finally {
        btnSalvar.disabled = false;
        btnSalvar.classList.remove('loading');
...
async function abrirModalEditarServico(nomeServico) {
    ...
    // Carregar duração e preço do serviço
    try {
        duracaoServico = await obterDuracaoServico(nomeServico);
        document.getElementById('inputDuracaoServico').value = duracaoServico;

        precoServico = await obterPrecoServico(nomeServico);
...
async function confirmarRemocaoServico() {
    ...
    try {
        ...
        operacaoPromise = (async () => {
            ...
            if (errorDuracao) {
                // Log silencioso para não inundar console
            }

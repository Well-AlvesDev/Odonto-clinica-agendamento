  // ===================================================
        ...
        function incrementarDiaNovo() {
            ...
            datePickerStateNovo.dia = (datePickerStateNovo.dia % diasNoMes) + 1;
        ...
        function incrementarMesNovo() {
            datePickerStateNovo.mes = (datePickerStateNovo.mes % 12) + 1;
            // Ajustar dia se necessário (ex: 31 de janeiro para fevereiro)
            const diasNoMes = new Date(datePickerStateNovo.ano, datePickerStateNovo.mes, 0).getDate();
        ...
        function inicializarPickerDeDataNovo(dataISO) {
            if (dataISO) {
                ...
                datePickerStateNovo.mes = hoje.getMonth() + 1;
                datePickerStateNovo.ano = hoje.getFullYear();
            }
            atualizarDatePickerDisplayNovo();
            sincronizarInputDateNovo();
            atualizarEstadoBotoesDatePickerNovo();
        ...
        function podeDecrementarMesNovo() {
            ...
            const dataMesAnterior = new Date(novoAno, novoMes - 1, datePickerStateNovo.dia);
            const dataHoje = new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate());
        ...
        function carregarDataAtual() {
            ...
            document.getElementById('editData').min = dataMinima;
        }
        ...
        function exibirHorarios() {
            ...
            const ativos = horariosClassificados.filter(h => h.status === 'Em breve');
            ...
            horariosOrdenados.forEach(horario => {
                ...
                card.innerHTML = `
                    ...
                    <div class="horario-actions">
                        <button class="btn-editar" onclick="abrirModalEdicao(${horario.id})" ${!podeEditar ? 'disabled' : ''}>
        ...
        function abrirModalEdicao(id) {
            ...
            // Inicializar o date picker com a data do registro
            inicializarPickerDeData(horario.data);
            atualizarEstadoBotoesDatePicker();

            // Parsear horários do string
            // Formato esperado: "HH:MM-HH:MM" para intervalo único ou "HH:MM-HH:MM, HH:MM-HH:MM" para múltiplos
            const horarioString = horario.horarios || '';
            const intervalos = horarioString.split(',').map(h => h.trim()).filter(h => h);

            // Limpar todos os inputs e pickers
            document.getElementById('editHoraInicio1').value = '';
            document.getElementById('editHoraFim1').value = '';
            document.getElementById('editHoraInicio2').value = '';
            document.getElementById('editHoraFim2').value = '';

            // Preencher os inputs com os intervalos encontrados
            if (intervalos.length >= 1) {
                const partes1 = intervalos[0].split('-');
                if (partes1.length === 2) {
                    document.getElementById('editHoraInicio1').value = partes1[0].trim();
                    document.getElementById('editHoraFim1').value = partes1[1].trim();
                }
            }

            if (intervalos.length >= 2) {
                const partes2 = intervalos[1].split('-');
                if (partes2.length === 2) {
                    document.getElementById('editHoraInicio2').value = partes2[0].trim();
                    document.getElementById('editHoraFim2').value = partes2[1].trim();
                }
            }

            // Inicializar os pickers a partir dos inputs (com valores do registro)
            inicializarPickerDeInput('inicio1', 'editHoraInicio1');
            inicializarPickerDeInput('fim1', 'editHoraFim1');
            inicializarPickerDeInput('inicio2', 'editHoraInicio2');
            inicializarPickerDeInput('fim2', 'editHoraFim2');

            // Carregar serviços no badge editor
            const servicosDoRegistro = obterServicosDoTexto(horario.servicos || horario.servico || '');
            servicosSelecionadosEdicao = servicosDoRegistro;
            document.getElementById('editServico').value = servicosDoRegistro.join(', ');
            renderizarBadgesServicos('servicosEditContainer', servicosSelecionadosEdicao);
